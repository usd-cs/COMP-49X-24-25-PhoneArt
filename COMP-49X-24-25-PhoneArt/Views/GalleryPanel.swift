       ...
       if isLoading {
           ...
           LazyVGrid(columns: columns, spacing: 16) {
               ForEach(sortedItems) { item in // ArtworkData is Identifiable
                   ArtworkGridItem(artwork: item, thumbnail: thumbnails[item.id]) // Pass thumbnail
                       .onTapGesture {
                           // Store the selected artwork and show confirmation
                           self.selectedArtworkForLoad = item
                           self.showingLoadConfirmation = true
                           print("Tapped on artwork: \(item.title ?? "Untitled"). Showing confirmation.")
                       }
               }
           }
           .padding()
